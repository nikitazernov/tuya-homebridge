...
class ActuatorAccessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      ...
      this.normalAsync(Characteristic.TargetHeaterCoolerState, 1)
      this.normalAsync(Characteristic.CurrentHeaterCoolerState, 1)
      ...
      if (statusMap.code === 'temp_set') {
        ...
        let tempsetval = this.tempsetMap.value/10;
        if (tempsetval < 10) {
          tempsetval = 10
        }

        this.normalAsync(Characteristic.TargetTemperature, tempsetval, {
